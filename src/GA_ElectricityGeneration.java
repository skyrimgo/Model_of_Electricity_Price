public class GA_ElectricityGeneration {
     ...
     * 将染色体转换成x,y变量的值
     */
    private double[] calculatefitnessvalue(String str, double q_1_max, double q_2_max, double q_3_max) {
        ...
        double z = c * (1.0 - 0.0) / (Math.pow(2, 23) - 1);

        // 需优化的函数
        // double fitness = 3 - Math.sin(2 * x) * Math.sin(2 * x) - Math.sin(2 * y) *
        // Math.sin(2 * y);
        double fitness = -((x - 0.07012) * (q_1_max * 1.2 - 2000 * x) + (y - 0.10455) * (q_2_max * 1.2 - 2000 * y)
                + (z - 0.163227) * (q_3_max * 1.2 - 2000 * z)) / 4;
     ...
     */
    private void select(double q_1_max, double q_2_max, double q_3_max) {
        double evals[] = new double[ChrNum]; // 所有染色体适应值
        double p[] = new double[ChrNum]; // 各染色体选择概率
        double q[] = new double[ChrNum]; // 累计概率
        double F = 0; // 累计适应值总和
        for (int i = 0; i < ChrNum; i++) {
            evals[i] = calculatefitnessvalue(ipop[i], q_1_max, q_2_max, q_3_max)[3];
    ...
    public double[] run(double q_1_max, double q_2_max, double q_3_max) {

        GA_ElectricityGeneration Tryer = new GA_ElectricityGeneration();
        Tryer.ipop = Tryer.initPop(); // 产生初始种群
        String str = "";

        // 迭代次数
        for (int i = 0; i < 100000; i++) {
            Tryer.select(q_1_max, q_2_max, q_3_max);
            Tryer.cross();
            Tryer.mutation();
            Tryer.generation = i;
        }

        double[] x = Tryer.calculatefitnessvalue(Tryer.beststr, q_1_max, q_2_max, q_3_max);

        str = "发电收益最大值" + -Tryer.bestfitness + "万元" + '\n' + "第" + Tryer.bestgenerations + "个染色体:<" + Tryer.beststr
                + ">" + '\n' + "电价分别为" + "x=" + x[0] + '\n' + "y=" + x[1] + '\n' + "z=" + x[2];

        System.out.println(str);
        double[] p_f_array = { x[0], x[1], x[2] };
        return p_f_array;
