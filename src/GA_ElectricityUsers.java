public class GA_ElectricityUsers {
    ...
    private double[] calculatefitnessvalue(String str) {
        ...
        double x = a * (1500.0 - 0.0) / (Math.pow(2, 23) - 1); // x的基因
        double y = b * (1500.0 - 0.0) / (Math.pow(2, 23) - 1); // y的基因
        double z = c * (1500.0 - 0.0) / (Math.pow(2, 23) - 1);

        // 需优化的函数
        // double fitness = 3 - Math.sin(2 * x) * Math.sin(2 * x) - Math.sin(2 * y) *
        // Math.sin(2 * y);
        double fitness = (0.15598*x+ 0.47453*y+ 0.71492*z)/(x+y+z);
    ...
    private void select() {
        ...
        double F = 0; // 累计适应值总和
        for (int i = 0; i < ChrNum; i++) {
            evals[i] = calculatefitnessvalue(ipop[i])[3];
    ...
    public static void main(String args[]) {
        ...
        GA_ElectricityUsers Tryer = new GA_ElectricityUsers();
